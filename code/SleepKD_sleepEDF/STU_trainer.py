...
from STU_sleepnet import STU_featurenet, STU_sleepnet
...
if __name__ == '__main__':
    ...
    for j in range(16, 21, 1):
        ...
        d.load_pretrain(i)
        pre_teacher = keras.models.load_model(join(path[0], f'pre_NOHARD_TA_sleepEDF_256_2.h5'))
        seq_teacher = keras.models.load_model(join(path[0], f'seq_NOHARD_TA_sleepEDF_256_2.h5'))
        ...
        # pre_model_saved.save(join(path[0], 'pre_FITNET_BASE_student_sleepEDF_' + str(n_lstms) + '_' + str(j) + '.h5'))
        # pre_model_saved.save(join(path[0], 'pre_epochLoss_student_sleepEDF_' + str(n_lstms) + '_' + str(j) + '.h5'))
        # pre_model_saved.save(join(path[0], 'pre_seqLoss_student_sleepEDF_' + str(n_lstms) + '_' + str(j) + '.h5'))
        # pre_model_saved.save(join(path[0], 'pre_logitLoss_student_sleepEDF_' + str(n_lstms) + '_' + str(j) + '.h5'))
        # pre_model_saved.save(join(path[0], 'pre_hardLoss_student_sleepEDF_' + str(n_lstms) + '_' + str(j) + '.h5'))
        # pre_model_saved.save(join(path[0], 'pre_T_student_sleepEDF_' + str(n_lstms) + '_' + str(j) + '.h5'))
        # pre_model_saved.save(join(path[0], 'pre_NOHARD_student_sleepEDF_' + str(n_lstms) + '_' + str(j) + '.h5'))
        ...
        y_pred = seq_model_saved.predict(d.X_seq_test)
        y_pred = y_pred.reshape((-1, 5))
        y_labels = d.y_seq_test.reshape((-1, 5))
        y_pred_argmax = y_pred.argmax(axis=1)
        y_labels_argmax = y_labels.argmax(axis=1)
        f1 = f1_score(y_labels_argmax, y_pred_argmax, average="macro")
        print(f"Macro F1-Score: {f1}")
        seq_f1_record.append(f1)

        # seq_model_saved.save(join(path[0], f'seq_FITNET_BASE_student_sleepEDF_' + str(n_lstms) + '_' + str(j) + '.h5'))
        # seq_model_saved.save(join(path[0], f'seq_epochLoss_student_sleepEDF_' + str(n_lstms) + '_' + str(j) + '.h5'))
        # seq_model_saved.save(join(path[0], f'seq_seqLoss_student_sleepEDF_' + str(n_lstms) + '_' + str(j) + '.h5'))
        # seq_model_saved.save(join(path[0], f'seq_logitLoss_student_sleepEDF_' + str(n_lstms) + '_' + str(j) + '.h5'))
        # seq_model_saved.save(join(path[0], f'seq_hardLoss_student_sleepEDF_' + str(n_lstms) + '_' + str(j) + '.h5'))
        # seq_model_saved.save(join(path[0], f'seq_T_student_sleepEDF_' + str(n_lstms) + '_' + str(j) + '.h5'))
        # seq_model_saved.save(join(path[0], f'seq_NOHARD_student_sleepEDF_' + str(n_lstms) + '_' + str(j) + '.h5'))
        # seq_model_saved.save(join(path[0], f'seq_NOHARD_student_sleepEDF_' + str(n_lstms) + '_' + str(j) + '.h5'))
        # seq_model_saved.save(join(path[0], f'seq_NOHARD_student_sleepEDF_' + str(n_lstms) + '_' + str(j) + '.h5'))
