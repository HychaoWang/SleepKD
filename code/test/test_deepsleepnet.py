...
def student_net(ch):
    Input_data = keras.Input(shape=(ch, 3000, 1), name="EEG/EOG_INPUT")
    padding = 'same'

    cov1 = Conv2D(filters=ch, kernel_size=(ch, 1), strides=(1, 1),
                  input_shape=(ch, 30 * 100, 1),
                  activation='linear')(Input_data)

    cov2 = Permute((3, 2, 1), input_shape=(1, 3000, 3))(cov1)

    cov3 = Conv2D(filters=8, kernel_size=(1, 64), strides=(1, 1), activation='relu', padding=padding)(cov2)
    ...
    cov6 = Conv2D(filters=8, kernel_size=(1, 64), strides=(1, 1), activation='relu', padding=padding)(cov5)

    cov7 = MaxPool2D(pool_size=(1, 16), strides=(1, 16))(cov6)

    cov8 = TimeDistributed(Flatten())(cov7)

    cov9 = Dropout(0.5)(cov8)
    cov10 = TimeDistributed(Dense(5, activation='softmax'))(cov9)
    model = Model(Input_data, cov10)
    model.summary()
    seq_opt = keras.optimizers.Adam(lr=1e-4)
    model.compile(loss='categorical_crossentropy', optimizer=seq_opt, metrics=['acc'])
    return model
