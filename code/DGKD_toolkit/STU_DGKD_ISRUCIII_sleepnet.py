...
from DistillationZoo.DGKD import DGKD_Loss
...
def STU_featurenet(n_filters=128):
    ...
    myLoss = DGKD_Loss()([true_label, T_label, TA_label, pre_softmax])

    pre_model = Model([input_signal, true_label, T_label, TA_label], [pre_softmax, myLoss])
...
def STU_sleepnet(pre_model, n_LSTM=512, context=20):
    ...
    TA_label = Input([None, 5], name="TA_label")
    ...
    myLoss = DGKD_Loss()([true_label, T_label, TA_label, seq_softmax])

    seq_model = Model([input_seq, true_label, T_label, TA_label], [seq_softmax, myLoss])
