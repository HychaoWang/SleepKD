...
from STU_DGKD_ISRUCIII_sleepnet import STU_featurenet, STU_sleepnet
...
if __name__ == '__main__':
    ...
    for j in range(5, 10, 1):
        ...
        d.load_pretrain(i)
        pre_teacher = keras.models.load_model(join(path[0], f'pre_teacher_ISRUCIII_4.h5'))
        seq_teacher = keras.models.load_model(join(path[0], f'seq_teacher_ISRUCIII_4.h5'))
        pre_TA = keras.models.load_model(join(path[0], f'pre_logitLoss_TA_ISRUCIII_256_4.h5'))
        seq_TA = keras.models.load_model(join(path[0], f'seq_logitLoss_TA_ISRUCIII_256_4.h5'))
        seq_teacher.summary()
        pre_teacher.summary()
        ...
        pre_train_data = [d.X_train, d.y_train, pre_teacher.predict(d.X_train),
                          pre_TA.predict(d.X_train)]
        pre_valid_data = [d.X_valid, d.y_valid, pre_teacher.predict(d.X_valid),
                          pre_TA.predict(d.X_valid)]
        pre_test_data = [d.X_test, d.y_test, pre_teacher.predict(d.X_test), pre_TA.predict(d.X_test)]
        ...
        pre_model_saved.save(os.path.join("../weights/Baseline", f'pre_DGKD_BASE_student_ISRUCIII_' + str(stu_lstms) + '_' + str(j) + '.h5'))
        ...
        seq_model_saved.save(os.path.join("../weights/Baseline/models", f'seq_DGKD_BASE_student_ISRUCIII_' + str(stu_lstms) + '_' + str(j) + '.h5'))
