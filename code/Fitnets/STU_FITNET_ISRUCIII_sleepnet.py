...
from DistillationZoo.MLKD import MLKDLoss
...
def STU_featurenet(n_filters=128):
    ...
    feature_map_label = Input(shape=1024, name="feature_map_label")
    ...
    myLoss = MLKDLoss(0, 0, 1)([true_label, soft_label, feature_map_label, feature_map, pre_softmax])

    pre_model = Model([input_signal, true_label, soft_label, feature_map_label], [pre_softmax, myLoss])
...
def STU_sleepnet(pre_model, n_LSTM=512, context=20):
    ...
    feature_map_label = Input(shape=(None, 2 * n_LSTM), name="feature_map_label")
    ...
    myLoss = MLKDLoss(1, 1, 0)([true_label, soft_label, feature_map_label, feature_map, seq_softmax])

    seq_model = Model([input_seq, true_label, soft_label, feature_map_label], [seq_softmax, myLoss])
