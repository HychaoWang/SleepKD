...
from tensorflow.keras.metrics import categorical_accuracy, KLDivergence, mean_squared_error
from sklearn import preprocessing
from copy import deepcopy
import numpy as np
from microsleepnet_master.DistillationZoo.DKD import DKD_seq, DKD_epoch
from microsleepnet_master.DistillationZoo.NST import mmd, MMDRegularizer
from microsleepnet_master.DistillationZoo.MLKD import MLKDLoss
...
def STU_featurenet(n_filters=128):
    ...
    myLoss = MLKDLoss(0, 1, 0.1)([true_label, soft_label, epoch_label, epoch_output, pre_softmax])
...
def STU_sleepnet(pre_model, n_LSTM=512, context=20):
    ...
    soft_label = Input([None, 5], name="soft_label")
    # epoch_label = Input([1024], name="epoch_label")
    sequence_label = Input([None, 2 * n_LSTM], name="sequence_label")
    ...
    dense_seq = Dense(5, name="classifier")(residual)
    ...
    myLoss = MLKDLoss(0, 1, 0.1)([true_label, soft_label, sequence_label, sequence_ouput, seq_softmax])
