...
def TA_featurenet(n_filters=128, context=20):
    ...
    myLoss = KD_Loss()([true_label, soft_label, pre_softmax])

    # print("epochShape")
    # print(epoch_label.shape)

    # pre_model = Model(input_signal,pre_softmax)
    pre_model = Model([input_signal, true_label, soft_label], [pre_softmax, myLoss])
...
def TAsleepnet(pre_model, n_LSTM=512, context=20):
    ...
    myLoss = KD_Loss()([true_label, soft_label, seq_softmax])

    seq_model = Model([input_seq, true_label, soft_label], [seq_softmax, myLoss])
